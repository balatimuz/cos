...
public class SCR_icon : MonoBehaviour {
	...
	void DisplayScale(){
		GetComponent<GUITexture>().pixelInset=new Rect(		(scaleDefault[0]*scale[0]*-0.5f),
	...
	void DisplayColour(){
		GetComponent<GUITexture>().color=new Color(colour.r,colour.g,colour.b,(alpha[0]*0.5f));
	}
