...
public class SCR_healthBar : MonoBehaviour {
	...
	void DisplayScale(int com){
		...
		if(com==0){
			GetComponent<GUITexture>().pixelInset=new Rect(		(scaleDefault[0]*-0.5f),
												(scaleDefault[1]*-0.5f),
												scaleDefault[0],
												scaleDefault[1]
			);
		}	else {
			bar.GetComponent<GUITexture>().pixelInset=new Rect(	(scaleDefault[0]*-0.5f),
												(scaleDefault[1]*-0.5f),
	...
	void DisplayColour(){
		GetComponent<GUITexture>().color=new Color(0.5f,0.5f,0.5f,(alpha[0]*0.5f));
		bar.GetComponent<GUITexture>().color=new Color(colour.x,colour.y,colour.z,(alpha[0]*0.5f));
	}
