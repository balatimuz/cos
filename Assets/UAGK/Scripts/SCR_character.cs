...
public class SCR_character : MonoBehaviour {
	...
	*/
	...
	void UpdateMovementFixed(){
		...
		GetComponent<Rigidbody>().MovePosition(new Vector3(GetComponent<Rigidbody>().transform.position.x,0f,GetComponent<Rigidbody>().transform.position.z)+(speed[0]*Time.deltaTime));
	}
	...
	public void PlayAnim(int com){
		...
		if(canRewind){
			if(animBlendSpeed[com]==0f||com==2||com>=5){
				GetComponent<Animation>().Rewind(animArray[com]);	
			}
		}
		
		if(animBlendSpeed[com]>0f){
			GetComponent<Animation>().CrossFade(animArray[com],animBlendSpeed[com]);	
		}	else {
			GetComponent<Animation>().Play (animArray[com]);
		}
		anim=com;
	}
	...
	void UpdateAnimation(){
		...
		if(rolling>0&&!GetComponent<Animation>().IsPlaying(animArray[4])){
			rolling=0;
		}
	}
	...
	public void SetPatrolSpeed(bool com){
		if(com){
			GetComponent<Animation>()[animArray[1]].speed=runAnimSpeedPatrol;
			runSpeed=runSpeedArray[1];
		}	else {
			GetComponent<Animation>()[animArray[1]].speed=animSpeed[1];
