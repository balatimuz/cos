...
public class SCR_option : MonoBehaviour {
	...
	void Awake(){
		...
		GetComponent<GUIText>().font=SCR_gui.tFont[fontType];
		GetComponent<GUIText>().material=SCR_gui.tFontMat[fontType];
	...
	void DisplayColour(){
		GetComponent<GUIText>().color=new Color(colour.x,colour.y,colour.z,alpha[0]);
	}
	...
	void SetSwitchText(){
		if(switchSoundFX){
			if(SCR_main.fxOn==1){
				GetComponent<GUIText>().text="Sound FX On";
			}	else {
				GetComponent<GUIText>().text="Sound FX Off";
			}
		}	else {
			if(switchMusic){
				if(SCR_main.musOn==1){
					GetComponent<GUIText>().text="Music On";
				}	else {
					GetComponent<GUIText>().text="Music Off";
				}
			}
		}
	}
