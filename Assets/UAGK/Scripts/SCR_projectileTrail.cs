...
public class SCR_projectileTrail : MonoBehaviour {
	...
	void Awake () {
		...
		if(UseAdditiveMaterial){
			GetComponent<Renderer>().material=SCR_main.particleMat[1];
		}	else {
			GetComponent<Renderer>().material=SCR_main.particleMat[0];
		}
		
		GetComponent<Renderer>().material.mainTexture=trailTex;
	...
	void DisplayColour(){
		GetComponent<Renderer>().material.SetColor("_TintColor",new Color(trailColor.r,trailColor.g,trailColor.b,(fadeCounter*trailAlpha*0.5f)));
	}
